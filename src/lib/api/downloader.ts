...
const COOKIES_PATH =
  process.env.COOKIES_PATH || path.join(process.cwd(), "cookies.txt");
...
async function searchYouTube(query: string): Promise<string | null> {
  try {
    ...
    try {
      ...
      console.log(
        `Found cookies file: ${COOKIES_PATH} (${cookieStats.size} bytes, modified: ${cookieStats.mtime})`,
      );
      ...
      lines.forEach((line, i) => {
        if (line.trim()) {
          console.log(
            `  ${i + 1}: ${line.substring(0, 100)}${line.length > 100 ? "..." : ""}`,
          );
        }
      });
    ...
    const data = JSON.parse(searchResults) as {
...
export async function downloadSong(
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "Could not create artist directory, using root download dir",
      );
    }

    const finalDownloadDir = await fs
      .access(artistDir)
      .then(() => artistDir)
      .catch(() => downloadDir);
    ...
    try {
      ...
      const cookieStats = await fs.stat(COOKIES_PATH);
      console.log(
        `Found cookies file for download: ${COOKIES_PATH} (${cookieStats.size} bytes)`,
      );
      ...
      console.log(
        "No cookies file found for download, proceeding without cookies",
      );
    }
